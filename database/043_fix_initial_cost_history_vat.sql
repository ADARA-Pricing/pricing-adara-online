...
as $$
  ...
  candidates as (
    ...
    select
      case
        when coalesce(h.previous_cost_without_vat, 0) = 0 then h.new_cost_without_vat
        else h.previous_cost_without_vat
      end as cost_without_vat,
      case
        when coalesce(h.previous_cost_without_vat, 0) = 0 then h.new_vat_rate
        else h.previous_vat_rate
      end as vat_rate,
      case
