...
alter table public.mercadolibre_order_items
  add column if not exists normalized_product_cost_without_vat numeric(14,2),
  add column if not exists normalized_product_vat_rate numeric(5,2);
...
create index if not exists idx_product_cost_history_sku_date
on public.product_cost_history (upper(sku), changed_at desc);
...
as $$
  ...
  candidates as (
    ...
    select
      h.previous_cost_without_vat as cost_without_vat,
      h.previous_vat_rate as vat_rate,
      'history_after'::text as source,
      h.changed_at as source_changed_at,
      2 as priority,
      h.changed_at as sort_at
    from public.product_cost_history h
...
as $$
  ...
  scoped as (
    select
      ...
      oi.unit_price,
      p.id as product_id,
      coalesce(pc.cost_without_vat, p.cost_without_vat, 0)::numeric as cost_without_vat,
      coalesce(pc.vat_rate, p.vat_rate, 21)::numeric as product_vat_rate,
    ...
    left join lateral public.product_cost_at(p.id, coalesce(oi.sku, p.sku), oi.order_date) pc on true
  ...
  updated as (
    ...
    set
      ...
      normalized_product_cost_without_vat = round(fv.cost_without_vat, 2),
      normalized_product_vat_rate = round(fv.product_vat_rate, 2),
