...
export default function AnalisisMercadoPage() {
  ...
  const [selected, setSelected] = useState<{ row: CompetitionRow; offers: RankedOffer[]; own?: { promo?: { name?: string | null; price?: number | null } | null } } | null>(null);
  ...
  return <main className="page market-analysis-page">
    ...
    {selected ? <div className="competition-modal-backdrop" onMouseDown={() => setSelected(null)}><section className="card competition-modal" onMouseDown={(event) => event.stopPropagation()}><div className="section-heading"><div><h2>Detalle de competencia · {selected.row.sku}</h2><p className="small">{selected.row.title}</p></div><button className="button ghost small-button" onClick={() => setSelected(null)}>Cerrar</button></div><div className="competition-detail-grid"><div>{selected.row.thumbnail ? <img className="competition-detail-image" src={selected.row.thumbnail} alt="" /> : null}<strong>{selected.row.publicationCount || 1} MLA de catálogo</strong></div><div><span>Precio 1 pago</span><strong>{moneyWithCents(selected.row.ownPrice)}</strong><span>Promo activa</span><strong>{selected.own?.promo ? `${selected.own.promo.name || "Promo"} · ${moneyWithCents(selected.own.promo.price)}` : "Sin promo"}</strong></div><div><span>Margen actual / sugerido</span><strong>{selected.row.marginAtSuggested !== undefined && selected.row.marginAtSuggested !== null ? `${selected.row.marginAtSuggested.toFixed(2)}% al precio para ganar` : "Sin cálculo"}</strong><span>Acción</span><strong>{selected.row.action?.replaceAll("_", " ") || "-"}</strong></div></div><h3>Top 5 · precio de 1 pago</h3><div className="table-wrap"><table className="market-results-table catalog-competition-table"><thead><tr><th>#</th><th>Vendedor</th><th>Precio</th><th>Factura</th><th>Full</th><th /></tr></thead><tbody>{selected.offers.map((offer, index) => <tr key={offer.itemId}><td>{index + 1}</td><td>{offer.isOwn ? <span className="badge success">Nosotros</span> : offer.nickname || offer.itemId}</td><td><strong>{moneyWithCents(offer.price)}</strong></td><td>{invoiceLabel(offer.invoiceA)}</td><td>{offer.full ? "Full" : "No Full"}</td><td>{offer.permalink ? <a href={offer.permalink} target="_blank" rel="noreferrer"><ExternalLink size={15} /></a> : "-"}</td></tr>)}</tbody></table></div></section></div> : null}
  </main>;
}
