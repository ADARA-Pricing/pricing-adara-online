...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) return NextResponse.json({ error: `Mercado Libre no pudo generar la hoja de control (${response.status}).` }, { status: 502 });
    const raw = Buffer.from(await response.arrayBuffer());
    if (raw.subarray(0, 2).toString() !== "PK") return NextResponse.json({ error: "Mercado Libre devolvió un archivo inesperado." }, { status: 502 });
    const source = await JSZip.loadAsync(raw);
    const control = Object.values(source.files).find((file) => !file.dir && /\.pdf$/i.test(file.name));
    if (!control) return NextResponse.json({ error: "Mercado Libre no incluyó la hoja de control PDF." }, { status: 502 });
